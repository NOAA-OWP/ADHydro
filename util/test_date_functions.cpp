...
#if !((DEBUG_LEVEL & DEBUG_LEVEL_PRIVATE_FUNCTIONS_SIMPLE) || (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE))
  // These functions don't work properly for negative years, and in fact calling the new functions with negative years throws an error.
  // However, if error checking is turned off we can still at least check that the new code gets the same answer as the old code.
...
#else
  printf("Test case 1: skipped\n");
#endif // !((DEBUG_LEVEL & DEBUG_LEVEL_PRIVATE_FUNCTIONS_SIMPLE) || (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE))
