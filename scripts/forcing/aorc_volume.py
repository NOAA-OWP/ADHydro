...
def getAORCCoords(aorc, cellSizeX=1000, cellSizeY=1000):
    ...
    for i in range(len(rows)):
        for j in range(len(cols)):
           ...
           aorcCells['row'].append(i)#len(rows)-i-1)
...
sub_basin_area = sub_basin['Area_mi2'][0] * 2.59e+6
...
def intersectAORC(s, AORCCoords, AORCIndex, bugFix):
    ...
    for i,p in ids:
        ...
        #cells['percent_intersecting'] = percent_intersecting
...
aorc_rows = np.sort(intersecting_ids['AORCrow'].unique())
aorc_cols = np.sort(intersecting_ids['AORCcol'].unique())
minX = aorc_cols.min()
minY = aorc_rows.min()
maxX = aorc_cols.max()
maxY = aorc_rows.max()
...
sub_aorc = aorcCoords.loc[sub_index]
"""
plt.figure()
ax = plt.gca()
print(aorcCoords)
...
if __name__ == '__main__':
    ...
    maxX = intersecting_ids['AORCcol'].max()
    ...
    for hour, times in time_groups:
        ...
        for i,t in enumerate(precip_times):
            ...
            if t.minute == 0 and not all_times.loc[t]['coincident']:
                ...
                precip_dataset = nc.Dataset(precip_file, 'r')
                fs_array_debug.append(np.flip(precip_dataset['APCP_surface'][:,:],0))
                precip_data[i] = np.flip(precip_dataset['APCP_surface'][:,:],0)[np.ix_(aorc_rows, aorc_cols)] #kg/m^2
            if i != 0:
                #Don't accumulate the first one since it really doesn't belong to the total accumulated, it accumulated before time t0
                """ 
                for r in sorted(aorc_rows):
                    #print(minY, maxY, '\t', minX, maxX)
                    for c in sorted(aorc_cols):
                        #print('\t', r, c, maxY-r, c-minX)
                        precip_data[i,maxY-r,c-minX] = precip_data[i,maxY-r,c-minX]# * aorcCoords.loc[(r,c)]['percent_intersecting']
                for idx in aorcCoords.index:
                    fs_array_debug[i][ idx[0], idx[1] ] *= aorcCoords.loc[idx]['percent_intersecting']
                """
                
                precip_data[i] *= np.reshape(aorcCoords.loc[sub_index]['percent_intersecting'].values, precip_data[i].shape)
                fs_array_debug[i] *= np.reshape(aorcCoords['percent_intersecting'].values, fs_array_debug[i].shape)
                ...
                """
                aorcCoords.loc[sub_index, 'precip'] =  precip_data[i].flatten()
                plt.figure()
                aorcCoords.loc[sub_index].plot('precip', ax=plt.gca(), edgecolor='none', legend=True)
                ex_left = aorcCoords.iloc[0].point.x
                ex_bottom = aorcCoords.iloc[-1].point.y
                ex_right = aorcCoords.iloc[-1].point.x
                ex_top = aorcCoords.iloc[0].point.y
                #img = plt.imshow(fs_array_debug[i], extent=[ex_left, ex_right, ex_bottom, ex_top], origin='lower')
                #plt.colorbar(img)
                sub_basin.plot(ax=plt.gca(), facecolor='none', edgecolor='red')
                plt.gca().set_title(t)
                plt.figure()
                aorcCoords['precip'] = fs_array_debug[i].flatten()
                aorcCoords.plot('precip', ax=plt.gca(), edgecolor='none', legend=True)
                sub_basin.plot(ax=plt.gca(), facecolor='none', edgecolor='red')
                plt.show()
                os._exit(1)
                """
            ...
            #print(fs2)
            plt.figure()
            ax = fs2.plot('precip',ax=plt.gca(), legend=True, edgecolor='green')
