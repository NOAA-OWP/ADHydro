...
# Creating new final matrix
final=finalold
if (mi==1){
rowsfinal=dim(final)[3]
exportname=final[1,2,1]
} else {
rowsfinal=dim(final)[3]+rowsfinal
}
...
dim2= ncdim_def( "Time",paste("Hours since ",final[1,2,1]),seq(1,(dim(final)[3])),unlim=TRUE) #final[1,2,]))
...
ncvar_put(nc.ex, varNODES, final[,which(ordencols=="node"),1])
}

ncvar_put(nc.ex, varJULTIME, final[1,which(ordencols=="JULTIME"),],start=tini,count=length(zt))
#ncvar_put(nc.ex, varWRFHOUR, final[1,which(ordencols=="WRFHOUR"),],start=tini,count=length(zt))
ncvar_put(nc.ex, varT2, final[,which(ordencols=="T2"),],start=c(1,tini),count=c(dim(TIN)[1],length(zt)))
ncvar_put(nc.ex, varQVAPOR, final[,which(ordencols=="QVAPOR"),],start=c(1,tini),count=c(dim(TIN)[1],length(zt)))
ncvar_put(nc.ex, varQCLOUD, as.double(final[,which(ordencols=="QCLOUD"),]),start=c(1,tini),count=c(dim(TIN)[1],length(zt)))
ncvar_put(nc.ex, varPSFC, final[,which(ordencols=="PSFC"),],start=c(1,tini),count=c(dim(TIN)[1],length(zt)))
ncvar_put(nc.ex, varU, final[,which(ordencols=="U"),],start=c(1,tini),count=c(dim(TIN)[1],length(zt)))
ncvar_put(nc.ex, varV, final[,which(ordencols=="V"),],start=c(1,tini),count=c(dim(TIN)[1],length(zt)))
ncvar_put(nc.ex, varVEGFRA, final[,which(ordencols=="VEGFRA"),],start=c(1,tini),count=c(dim(TIN)[1],length(zt)))
ncvar_put(nc.ex, varTPREC, final[,which(ordencols=="TPREC"),],start=c(1,tini),count=c(dim(TIN)[1],length(zt)))
ncvar_put(nc.ex, varSWDOWN, final[,which(ordencols=="SWDOWN"),],start=c(1,tini),count=c(dim(TIN)[1],length(zt)))
ncvar_put(nc.ex, varGLW, final[,which(ordencols=="GLW"),],start=c(1,tini),count=c(dim(TIN)[1],length(zt)))
ncvar_put(nc.ex, varPBLH, final[,which(ordencols=="PBLH"),],start=c(1,tini),count=c(dim(TIN)[1],length(zt)))
ncvar_put(nc.ex, varTSLB, final[,which(ordencols=="TSLB"),],start=c(1,tini),count=c(dim(TIN)[1],length(zt)))

}

### Before closing the file, compute MAXVEGFRAC if requested
if (length(which(Outnames=="MAXVEGFRA"))>0){
    cat("Adding MAXVEGFRA to final netcdf file",fill=TRUE)
...
Avefinal=matrix(NA,rowsfinal,(dim(final)[2]-3)) ## matrix containing the variables in the columns (not includign  node, WRFHOUR or JULTIME) and times in the rows
