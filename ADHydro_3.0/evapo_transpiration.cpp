...
bool evapoTranspirationInit(const char* mpTableFile, const char* vegParmFile, const char* soilParmFile, const char* genParmFile)
{
  ...
  int         verbosityLevel = Readonly::verbosityLevel; // For passing verbosity level into Noah-MP.  I'm not comfortable passing a class static variable by reference into foreign code.
  ...
  if (!(NULL != mpTableFile))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationInit: mpTableFile must not be NULL.\n");
      error = true;
    }
  
  if (!(NULL != vegParmFile))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationInit: vegParmFile must not be NULL.\n");
      error = true;
    }
  
  if (!(NULL != soilParmFile))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationInit: soilParmFile must not be NULL.\n");
      error = true;
    }
  
  if (!(NULL != genParmFile))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationInit: genParmFile must not be NULL.\n");
      error = true;
    }
                            ...
                            float* evaporationFromSnow, float* evaporationFromGround, float* transpiration, float* waterError)
{
  ...
  if (!(1 <= vegType && 27 >= vegType))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: USGS vegetation type must be greater than or equal to 1 and less than or equal to 27.\n");
      error = true;
    }
  ...
  if (16 == vegType)
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in evapoTranspirationSoil: vegetation type is 'Water Bodies'.  Call evapoTranspirationWater instead.\n");
        }
    }
  ...
  if (24 == vegType)
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in evapoTranspirationSoil: vegetation type is 'Snow or Ice'.  Call evapoTranspirationGlacier instead.\n");
        }
    }

  if (!(1 <= soilType && 19 >= soilType))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: STAS soil type must be greater than or equal to 1 and less than or equal to 19.\n");
      error = true;
    }
  ...
  if (14 == soilType)
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in evapoTranspirationSoil: soil type is 'WATER'.  Call evapoTranspirationWater instead.\n");
        }
    }
  ...
  if (16 == soilType)
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in evapoTranspirationSoil: soil type is 'OTHER(land-ice)'.  Call evapoTranspirationGlacier instead.\n");
        }
    }
  
  if (!(-M_PI / 2.0f <= lat && M_PI / 2.0f >= lat))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: lat must be greater than or equal to -pi/2 and less than or equal to pi/2.\n");
      error = true;
    }
  
  if (!(365 == yearLen || 366 == yearLen))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: yearLen must be 365 or 366.\n");
      error = true;
    }
  
  if (!(0.0f <= julian && julian <= yearLen))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: julian must be greater than or equal to zero and less than or equal to yearLen.\n");
      error = true;
    }
  
  if (!(0.0f <= cosZ && 1.0f >= cosZ))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: cosZ must be greater than or equal to zero and less than or equal to one.\n");
      error = true;
    }
  
  if (!(0.0f < dt))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: dt must be greater than zero.\n");
      error = true;
    }
  
  if (!(0.0f < dx))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: dx must be greater than zero.\n");
      error = true;
    }
  
  if (!(NULL != evapoTranspirationForcing))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: evapoTranspirationForcing must not be NULL.\n");
      error = true;
    }
  ...
  if (!(NULL != evapoTranspirationState))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: evapoTranspirationState must not be NULL.\n");
      error = true;
    }

  if (!(NULL != surfacewaterAdd))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: surfacewaterAdd must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!(NULL != evaporationFromCanopy))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: evaporationFromCanopy must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!(NULL != evaporationFromSnow))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: evaporationFromSnow must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!(NULL != evaporationFromGround))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: evaporationFromGround must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!(NULL != transpiration))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: transpiration must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!(NULL != waterError))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationSoil: waterError must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!error)
    {
      ...
      ADHYDRO_ASSERT(iLocOriginal == iLoc && jLocOriginal == jLoc && latOriginal == lat && yearLenOriginal == yearLen && julianOriginal == julian &&
      ...
      if (0.0f != ponding)
        {
          ADHYDRO_ASSERT(epsilonEqual(snEqvOriginal, evapoTranspirationState->snEqvO + ponding));
        }
      else
        {
          ADHYDRO_ASSERT(snEqvOriginal == evapoTranspirationState->snEqvO);
        }
      ...
      ADHYDRO_ASSERT(0.0f <= fpIce && 1.0f >= fpIce && 0.0f <= qSnow && 0.0f <= qRain && 0.0f <= qSnBot && 0.0f <= ponding && 0.0f <= ponding1 && 0.0f <= ponding2);
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
      ...
      *evaporationFromCanopy = eCan * dt;
      ...
      if (0.0f > snEqvShouldBe)
        {
#if (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
          ADHYDRO_ASSERT(epsilonEqual(0.0f, snEqvShouldBe));
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
      ...
      if (0.0f == evapoTranspirationState->snEqv)
        {
#if (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
          ADHYDRO_ASSERT(epsilonGreaterOrEqual(0.001f, snEqvShouldBe));
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
...
#if (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
          ADHYDRO_ASSERT(epsilonGreaterOrEqual(evapoTranspirationState->snEqv + runoff, snEqvShouldBe));
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
        ...
        {
#if (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
          ADHYDRO_ASSERT(epsilonEqual(evapoTranspirationState->snEqv, snEqvShouldBe));
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
        }

#if (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_INVARIANTS)
      ADHYDRO_ASSERT(!checkEvapoTranspirationStateStructInvariant(evapoTranspirationState));
...
#if (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
      ADHYDRO_ASSERT(0.0f <= *surfacewaterAdd && 0.0f <= *transpiration);
                             ...
                             float* surfacewaterAdd, float* evaporationFromSnow, float* evaporationFromGround, float* waterError)
{
  ...
  if (!(-M_PI / 2.0f <= lat && M_PI / 2.0f >= lat))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: lat must be greater than or equal to -pi/2 and less than or equal to pi/2.\n");
      error = true;
    }
  
  if (!(365 == yearLen || 366 == yearLen))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: yearLen must be 365 or 366.\n");
      error = true;
    }
  
  if (!(0.0f <= julian && julian <= yearLen))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: julian must be greater than or equal to zero and less than or equal to yearLen.\n");
      error = true;
    }
  
  if (!(0.0f <= cosZ && 1.0f >= cosZ))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: cosZ must be greater than or equal to zero and less than or equal to one.\n");
      error = true;
    }
  
  if (!(0.0f < dt))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: dt must be greater than zero.\n");
      error = true;
    }
  
  if (!(0.0f < dx))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: dx must be greater than zero.\n");
      error = true;
    }

  if (!(0.0f <= wsLake))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: wsLake must be greater than or equal to zero.\n");
      error = true;
    }
  
  if (!(NULL != evapoTranspirationForcing))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: evapoTranspirationForcing must not be NULL.\n");
      error = true;
    }
  
  if (!(NULL != evapoTranspirationState))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: evapoTranspirationState must not be NULL.\n");
      error = true;
    }
  else
    {
      if (!(0.0f == evapoTranspirationState->canLiq))
        {
          ADHYDRO_ERROR("ERROR in evapoTranspirationWater: canLiq must be zero.\n");
          error = true;
        }

      if (!(0.0f == evapoTranspirationState->canIce))
        {
          ADHYDRO_ERROR("ERROR in evapoTranspirationWater: canIce must be zero.\n");
          error = true;
        }
    }
  ...
  if (!(NULL != surfacewaterAdd))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: surfacewaterAdd must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!(NULL != evaporationFromSnow))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: evaporationFromSnow must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!(NULL != evaporationFromGround))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: evaporationFromGround must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!(NULL != waterError))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationWater: waterError must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!error)
    {
      ...
      ADHYDRO_ASSERT(0.0f == evapoTranspirationState->canLiq && 0.0f == evapoTranspirationState->canIce && 0.0f == eCan && 0.0f == eTran);
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
      ...
      if (NOAHMP_TFRZ < evapoTranspirationState->tg)
      ...
      else
        {
          ...
          ADHYDRO_ASSERT(epsilonEqual(evapoTranspirationForcing->prcp * fpIce, qSnow));
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
        }
      
      // Verify that there is no rainfall interception in the non-existant canopy.
      ADHYDRO_ASSERT(epsilonEqual(evapoTranspirationForcing->prcp * (1.0f - fpIce), qRain));
      ...
      if (0.0f == evapoTranspirationState->snEqv)
        {
#if (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
          ADHYDRO_ASSERT(epsilonGreaterOrEqual(0.001f + thrownAwaySnow, snEqvShouldBe));
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
...
#if (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
      ADHYDRO_ASSERT(0.0f <= *surfacewaterAdd);
                               ...
                               float* evaporationFromGround, float* waterError)
{
  ...
  if (!(0.0f <= cosZ && 1.0f >= cosZ))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationGlacier: cosZ must be greater than or equal to zero and less than or equal to one.\n");
      error = true;
    }
  
  if (!(0.0f < dt))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationGlacier: dt must be greater than zero.\n");
      error = true;
    }
  
  if (!(NULL != evapoTranspirationForcing))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationGlacier: evapoTranspirationForcing must not be NULL.\n");
      error = true;
    }

  if (!(NULL != evapoTranspirationState))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationGlacier: evapoTranspirationState must not be NULL.\n");
      error = true;
    }
  else
    {
      if (!(0.0f == evapoTranspirationState->canLiq))
        {
          ADHYDRO_ERROR("ERROR in evapoTranspirationGlacier: canLiq must be zero.\n");
          error = true;
        }

      if (!(0.0f == evapoTranspirationState->canIce))
        {
          ADHYDRO_ERROR("ERROR in evapoTranspirationGlacier: canIce must be zero.\n");
          error = true;
        }
    }
  ...
  if (!(NULL != surfacewaterAdd))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationGlacier: surfacewaterAdd must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!(NULL != evaporationFromSnow))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationGlacier: evaporationFromSnow must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!(NULL != evaporationFromGround))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationGlacier: evaporationFromGround must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!(NULL != waterError))
    {
      ADHYDRO_ERROR("ERROR in evapoTranspirationGlacier: water error must not be NULL.\n");
      error = true;
    }
  else
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
  ...
  if (!error)
    {
...
#if (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
      // Verify that the input variables have not changed.
      ADHYDRO_ASSERT(iLocOriginal == iLoc && jLocOriginal == jLoc && cosZOriginal == cosZ && nSnowOriginal == nSnow && nSoilOriginal == nSoil && dtOriginal == dt &&
                     sfcTmpOriginal == evapoTranspirationForcing->sfcTmp && sfcPrsOriginal == evapoTranspirationForcing->sfcPrs &&
      ...
      ADHYDRO_ASSERT(0.0f <= fpIce && 1.0f >= fpIce && 0.0f <= qSnow && 0.0f <= qSnBot && 0.0f <= ponding && 0.0f <= ponding1 && 0.0f <= ponding2);
      
      // Verify that there is no snowfall interception in the non-existant canopy.
      ADHYDRO_ASSERT(evapoTranspirationForcing->prcp * fpIce == qSnow);
#endif // (DEBUG_LEVEL & DEBUG_LEVEL_INTERNAL_SIMPLE)
...
bool checkEvapoTranspirationForcingStructInvariant(const EvapoTranspirationForcingStruct* evapoTranspirationForcing)
{
  ...
  if (!(0.0f < evapoTranspirationForcing->dz8w))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationForcingStructInvariant: dz8w must be greater than zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationForcing->sfcTmp))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationForcingStructInvariant: sfcTmp must be greater than or equal to zero.\n");
      error = true;
    }
  else if (!(-70.0f + ZERO_C_IN_KELVIN <= evapoTranspirationForcing->sfcTmp))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationForcingStructInvariant: sfcTmp below -70 degrees C.\n");
        }
    }
  else if (!(70.0f + ZERO_C_IN_KELVIN >= evapoTranspirationForcing->sfcTmp))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationForcingStructInvariant: sfcTmp above 70 degrees C.\n");
        }
    }

  if (!(0.0f <= evapoTranspirationForcing->sfcPrs))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationForcingStructInvariant: sfcPrs must be greater than or equal to zero.\n");
      error = true;
    }
  else if (!(35000.0f <= evapoTranspirationForcing->sfcPrs))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationForcingStructInvariant: sfcPrs below 35 kPa.\n");
        }
    }

  if (!(0.0f <= evapoTranspirationForcing->psfc))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationForcingStructInvariant: psfc must be greater than or equal to zero.\n");
      error = true;
    }
  else if (!(35000.0f <= evapoTranspirationForcing->psfc))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationForcingStructInvariant: psfc below 35 kPa.\n");
        }
    }
  ...
  if (!(100.0f >= fabs(evapoTranspirationForcing->uu)))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationForcingStructInvariant: magnitude of uu greater than 100 m/s.\n");
        }
    }
  ...
  if (!(100.0f >= fabs(evapoTranspirationForcing->vv)))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationForcingStructInvariant: magnitude of vv greater than 100 m/s.\n");
        }
    }
  
  if (!(0.0f <= evapoTranspirationForcing->q2 && 1.0f >= evapoTranspirationForcing->q2))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationForcingStructInvariant: q2 must be greater than or equal to zero and less than or equal to one.\n");
      error = true;
    }
  
  if (!(0.0f <= evapoTranspirationForcing->qc && 1.0f >= evapoTranspirationForcing->qc))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationForcingStructInvariant: qc must be greater than or equal to zero and less than or equal to one.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationForcing->solDn))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationForcingStructInvariant: solDn must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationForcing->lwDn))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationForcingStructInvariant: lwDn must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationForcing->prcp))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationForcingStructInvariant: prcp must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationForcing->tBot))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationForcingStructInvariant: tBot must be greater than or equal to zero.\n");
      error = true;
    }
  else if (!(-70.0f + ZERO_C_IN_KELVIN <= evapoTranspirationForcing->tBot))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationForcingStructInvariant: tBot below -70 degrees C.\n");
        }
    }
  else if (!(70.0f + ZERO_C_IN_KELVIN >= evapoTranspirationForcing->tBot))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationForcingStructInvariant: tBot above 70 degrees C.\n");
        }
    }

  if (!(0.0f <= evapoTranspirationForcing->pblh))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationForcingStructInvariant: pblh must be greater than or equal to zero.\n");
      error = true;
    }
...
bool checkEvapoTranspirationStateStructInvariant(const EvapoTranspirationStateStruct* evapoTranspirationState)
{
  ...
  for (ii = 0; ii < EVAPO_TRANSPIRATION_NUMBER_OF_SNOW_LAYERS; ii++)
    {
      if (!(0.0f <= evapoTranspirationState->fIceOld[ii] && evapoTranspirationState->fIceOld[ii] <= 1.0f))
        {
          ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant, snow layer %d: fIceOld must be greater than or equal to zero and less than or equal to one.\n", ii);
          error = true;
        }
    }
  ...
  if (!(0.0f <= evapoTranspirationState->albOld && evapoTranspirationState->albOld <= 1.0f))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: albOld must be greater than or equal to zero and less than or equal to one.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->snEqvO))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: snEqvO must be greater than or equal to zero.\n");
      error = true;
    }
      ...
      if (ii < evapoTranspirationState->iSnow + EVAPO_TRANSPIRATION_NUMBER_OF_SNOW_LAYERS)
        {
          ...
          if (!(0.0f == evapoTranspirationState->stc[ii]))
            {
              ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant, non-existant snow layer %d: stc must be equal to zero.\n", ii);
              error = true;
            }
        }
      else
        {
          ...
          if (!(0.0f <= evapoTranspirationState->stc[ii]))
            {
              ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant, snow/soil layer %d: stc must be greater than or equal to zero.\n", ii);
              error = true;
            }
          else if (!(-70.0f + ZERO_C_IN_KELVIN <= evapoTranspirationState->stc[ii]))
            {
              if (2 <= Readonly::verbosityLevel)
                {
                  ADHYDRO_ERROR("WARNING in checkEvapoTranspirationStateStructInvariant, snow/soil layer %d: stc below -70 degrees C.\n", ii);
                }
            }
          else if (!(70.0f + ZERO_C_IN_KELVIN >= evapoTranspirationState->stc[ii]))
            {
              if (2 <= Readonly::verbosityLevel)
                {
                  ADHYDRO_ERROR("WARNING in checkEvapoTranspirationStateStructInvariant, snow/soil layer %d: stc above 70 degrees C.\n", ii);
                }
            }
        }
    }
  ...
  if (!(0.0f <= evapoTranspirationState->tah))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: tah must be greater than or equal to zero.\n");
      error = true;
    }
  else if (!(-70.0f + ZERO_C_IN_KELVIN <= evapoTranspirationState->tah))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationStateStructInvariant: tah below -70 degrees C.\n");
        }
    }
  else if (!(70.0f + ZERO_C_IN_KELVIN >= evapoTranspirationState->tah))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationStateStructInvariant: tah above 70 degrees C.\n");
        }
    }

  if (!(0.0f <= evapoTranspirationState->eah))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: eah must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->fWet && evapoTranspirationState->fWet <= 1.0f))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: fWet must be greater than or equal to zero and less than or equal to one.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->canLiq))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: canLiq must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->canIce))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: canIce must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->tv))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: tv must be greater than or equal to zero.\n");
      error = true;
    }
  else if (!(-70.0f + ZERO_C_IN_KELVIN <= evapoTranspirationState->tv))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationStateStructInvariant: tv below -70 degrees C.\n");
        }
    }
  else if (!(70.0f + ZERO_C_IN_KELVIN >= evapoTranspirationState->tv))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationStateStructInvariant: tv above 70 degrees C.\n");
        }
    }

  if (!(0.0f <= evapoTranspirationState->tg))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: tg must be greater than or equal to zero.\n");
      error = true;
    }
  else if (!(-70.0f + ZERO_C_IN_KELVIN <= evapoTranspirationState->tg))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationStateStructInvariant: tg below -70 degrees C.\n");
        }
    }
  else if (!(70.0f + ZERO_C_IN_KELVIN >= evapoTranspirationState->tg))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationStateStructInvariant: tg above 70 degrees C.\n");
        }
    }

  if (!(-EVAPO_TRANSPIRATION_NUMBER_OF_SNOW_LAYERS <= evapoTranspirationState->iSnow && evapoTranspirationState->iSnow <= 0))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: iSnow must be greater than or equal to minus three and less than or equal to zero.\n");
      error = true;
    }
      ...
      if (ii < evapoTranspirationState->iSnow + EVAPO_TRANSPIRATION_NUMBER_OF_SNOW_LAYERS)
        {
          ...
          if (!(0.0f == evapoTranspirationState->zSnso[ii]))
            {
              ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant, non-existant snow layer %d: zSnso must be equal to zero.\n", ii);
              error = true;
            }

          if (!(0.0f == evapoTranspirationState->snIce[ii]))
            {
              ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant, non-existant snow layer %d: snIce must be equal to zero.\n", ii);
              error = true;
            }

          if (!(0.0f == evapoTranspirationState->snLiq[ii]))
            {
              ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant, non-existant snow layer %d: snLiq must be equal to zero.\n", ii);
              error = true;
            }
        }
      else
        {
          ...
          if (ii == evapoTranspirationState->iSnow + EVAPO_TRANSPIRATION_NUMBER_OF_SNOW_LAYERS)
            {
              ...
              if (!(0.0f > evapoTranspirationState->zSnso[ii]))
                {
                  ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant, snow layer %d: zSnso must be less than zero.\n", ii);
                  error = true;
                }
            }
          else
            {
              ...
              if (!(evapoTranspirationState->zSnso[ii - 1] > evapoTranspirationState->zSnso[ii]))
                {
                  ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant, snow layer %d: zSnso must be less than the layer above it.\n", ii);
                  error = true;
                }
            }
          ...
          if (!(0.0f < evapoTranspirationState->snIce[ii]))
            {
              ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant, snow layer %d: snIce must be greater than zero.\n", ii);
              error = true;
            }

          if (!(0.0f <= evapoTranspirationState->snLiq[ii]))
            {
              ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant, snow layer %d: snLiq must be greater than or equal to zero.\n", ii);
              error = true;
            }
  ...
  for (ii = EVAPO_TRANSPIRATION_NUMBER_OF_SNOW_LAYERS; ii < EVAPO_TRANSPIRATION_NUMBER_OF_ALL_LAYERS; ii++)
    {
      if (!(evapoTranspirationState->zSnso[ii - 1] > evapoTranspirationState->zSnso[ii]))
        {
          ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant, soil layer %d: zSnso must be less than the layer above it.\n", ii - EVAPO_TRANSPIRATION_NUMBER_OF_SNOW_LAYERS);
          error = true;
        }
    }
  ...
  if (!(0.0f <= evapoTranspirationState->snowH))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: snowH must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->snEqv))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: snEqv must be greater than or equal to zero.\n");
      error = true;
    }
  ...
  if (0 > evapoTranspirationState->iSnow)
    {
      if (!epsilonEqual(-evapoTranspirationState->zSnso[EVAPO_TRANSPIRATION_NUMBER_OF_SNOW_LAYERS - 1], evapoTranspirationState->snowH))
        {
          ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: snowH must be consistent with zSnso.\n");
          error = true;
        }

      if (!epsilonEqual(totalSnowWater, evapoTranspirationState->snEqv))
        {
          ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: snEqv must be consistent with snIce and snLiq.\n");
          error = true;
        }
    }
  ...
  if (!(0.0f <= evapoTranspirationState->lfMass))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: lfMass must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->rtMass))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: rtMass must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->stMass))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: stMass must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->wood))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: wood must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->stblCp))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: stblCp must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->fastCp))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: fastCp must be greater than or equal to zero.\n");
      error = true;
    }

  if (!(0.0f <= evapoTranspirationState->lai))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: lai must be greater than or equal to zero.\n");
      error = true;
    }
  else if (!(6.0f >= evapoTranspirationState->lai))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationStateStructInvariant: lai above six.\n");
        }
    }

  if (!(0.0f <= evapoTranspirationState->sai))
    {
      ADHYDRO_ERROR("ERROR in checkEvapoTranspirationStateStructInvariant: sai must be greater than or equal to zero.\n");
      error = true;
    }
  else if (!(1.6f >= evapoTranspirationState->sai))
    {
      if (2 <= Readonly::verbosityLevel)
        {
          ADHYDRO_ERROR("WARNING in checkEvapoTranspirationStateStructInvariant: sai above 1.6.\n");
        }
    }
