...
outputManager(outputManagerInit)
{
  if (DEBUG_LEVEL & DEBUG_LEVEL_PUBLIC_FUNCTIONS_SIMPLE)
    {
      if (!(NULL != outputManagerInit))
        {
          ADHYDRO_ERROR("ERROR in FileManagerNetCDF::FileManagerNetCDF: outputManagerInit must not be NULL.\n");
          ADHYDRO_EXIT;
        }
    }
}
...
bool FileManagerNetCDF::createFile(double outputTime)
{
  ...
  std::ostringstream filename;                      // Filename of the file to create.
  ...
  if (!error)
    {
      ...
      ncErrorCode = nc_create(filename.str().c_str(), NC_NETCDF4 | NC_WRITE | NC_NOCLOBBER, &fileID);
  ...
  if (!error)
    {
      ncErrorCode = nc_def_dim(fileID, "meshSoilLayers", outputManager->numberOfMeshSoilLayers(), &meshSoilLayersDimensionID);
  ...
  if (!error)
    {
      ncErrorCode = nc_def_dim(fileID, "meshNeighbors", outputManager->numberOfMeshNeighbors(), &meshNeighborsDimensionID);
  ...
  if (!error)
    {
      ncErrorCode = nc_def_dim(fileID, "channelNeighbors", outputManager->numberOfChannelNeighbors(), &channelNeighborsDimensionID);
  ...
  if (!error)
    {
      ncErrorCode = nc_def_opaque(fileID, 10 /* FIXME sizeof(EvapoTranspirationStateStruct) */, "EvapoTranspirationState", &EvapoTranspirationStateTypeID);
  ...
  if (!error)
    {
      ncErrorCode = nc_def_opaque(fileID, 10 /* FIXME sizeof(EvapoTranspirationStateStruct) */, "VadoseZoneState", &VadoseZoneStateTypeID);
